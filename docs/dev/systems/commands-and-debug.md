...
| `alts` | `on`, `off`, `toggle`, `follow [name|off]`, `do <command>`, `mirror`, `window` | Orders to every other member of the box group (`DualBoxConfig.group`), from either box | `AltGroup.route` -> `dualbox/alt_group.lua` (see [dual-box](dualbox.md)) |
