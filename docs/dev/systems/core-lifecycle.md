...
- Timeout (`calculate_timeout`, `:86-114`): items use `res.items[id].cast_delay` (or `cast_time`) + buffer with no Fast Cast; spells use `res.spells[id].cast_time * (1 - FC/100) + buffer`, where FC is `state.FastCast.value` capped at 80 (`get_fast_cast_percent`, `:71-84`). Unknown id: `WATCHDOG_FALLBACK_TIMEOUT` (5.0 s). Buffer `WATCHDOG_BUFFER` 1.5 s. Since 2026-09-25 `on_midcast_start` replaces that estimate by `_G._precast_cast_time` when it belongs to the same spell id: the cast time computed from the precast set GearSwap actually sent (`shared/utils/precast/cast_time.lua`, below).
...
**Cast time from the gear** (`shared/utils/precast/cast_time.lua`, 2026-09-25). `CastTime.install_hook()` (INIT_SYSTEMS) wraps `cleanup_precast`: for a magic action not cancelled, the worn gear (`player.equipment`) with this precast's `gearswap.equip_list` over it is summed from each piece's `res.item_descriptions` text and the augments written in the sets: `"Fast Cast"+N`, and every `<kind> (spell)casting time -N%` matching the spell (song, cure, utsusemi, stoneskin, elemental, healing, enhancing, enfeebling, dark, divine, blue, or all spells; `Grimoire:` only under the matching Arts). Plus the RDM trait by level (10-30, job points not counted), capped at 80 %; songs x0.5 under Nightingale and x1.5 under Troubadour; Celerity / Alacrity x0.5 (BG-Wiki). Haste does not shorten a cast. Not counted: set bonuses, latents, Quick Magic. Stored in `_G._precast_cast_time = {id, name, seconds, percent}`.
