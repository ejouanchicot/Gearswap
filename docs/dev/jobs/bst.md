...
`shared/jobs/bst/functions/` (2 623 lines), one entry point, nine config files
...
(structure and set names only; gear choice is out of scope). Line numbers were
rechecked against the working tree on 2026-09-25; where a line number added
nothing, the function name is cited instead. GearSwap engine and Mote-Include
line numbers refer to `D:\Windower Tetsouo\addons\GearSwap\` (outside the repo).
...
| `shared/jobs/bst/functions/BST_PET_PRECAST.lua` | 105 | `job_pet_precast`: never called (no such hook in Mote or GearSwap) |
...
| `shared/jobs/bst/functions/BST_COMMANDS.lua` | 497 | `job_self_command` router; `job_state_change = LifecycleManager.state_change()` |
| `shared/jobs/bst/functions/BST_MOVEMENT.lua` | 41 | Empty `job_handle_equipping_gear` |
| `shared/jobs/bst/functions/BST_LOCKSTYLE.lua` | 49 | Lazy `LockstyleManager.create('BST', ...)` wrappers |
| `shared/jobs/bst/functions/BST_MACROBOOK.lua` | 43 | Lazy `MacrobookManager.create('BST', ...)` wrapper |
| `shared/jobs/bst/functions/logic/ecosystem_manager.lua` | 262 | Rebuilds `state.species` / `state.ammoSet`, broth "equip", jug count |
| `shared/jobs/bst/functions/logic/pet_manager.lua` | 317 | Pet-valid cache, auto-engage, engage/disengage, Ready-move list (30 s cache), `monitor_pet_status` |
| `shared/jobs/bst/functions/logic/ready_move_categorizer.lua` | 283 | Four name lists (`S{}`) covering the 120 Ready moves, `get_category`, `_G.pet*Moves` exports |
| `shared/jobs/bst/functions/logic/set_builder.lua` | 232 | Idle/engaged set construction (pet vs master, PDT overlays, weapons, movement, town feet) |
| `_master/config/bst/BST_STATES.lua` | 77 | `BSTStates.configure()` |
| `_master/config/bst/BST_KEYBINDS.lua` | 48 | 8 binds, data only; `KeybindManager.create('BST', ...)` adds `bind_all` / `unbind_all` / `show_intro` (see [keybinds and custom states](../systems/keybinds-and-custom.md)) |
| `_master/config/bst/BST_CUSTOM.lua` | 118 | Player modes and gear rules (all examples commented out), read through `KeybindManager` |
| `_master/config/bst/BST_LOCKSTYLE.lua` | 65 | Lockstyle 6 (`default`, `by_subjob`, `get_style`) |
| `_master/config/bst/BST_MACROBOOK.lua` | 61 | Book/page per subjob and per dual-box partner job |
| `_master/config/bst/BST_PET_DATA.lua` | 179 | 25 jug pets (broth, species, ecosystem, job) and three list getters -> `_G.BSTBeastPetData` |
| `_master/config/bst/BST_TP_CONFIG.lua` | 172 | Moonshade piece, Fencer bonus -> `_G.BSTTPConfig` |
...
`BST_STATES.lua:31` (`Ecosystem` default `Amorph` instead of `Aquan`); both
live versions are also in `_master/Tetsouo/config/bst/`, and `BST_REFILL.lua`
is identical to that overlay.
`Tetsouo/sets/bst/{bst_sets,armor,capes,pets,weapons}.lua` are the modular live
sets (509 + 238 + 77 + 45 + 32 lines), mirrored in `_master/Tetsouo/sets/bst/`.
`Kaories/` and `_master/Kaories/` have no BST files.
...
(`Mote-Include.lua:170-175`), before `INIT_SYSTEMS` (included on the next line
of `get_sets`) and before any BST hook file exists. See
[job change lifecycle](../architecture/job-change-lifecycle.md) for
...
sequenceDiagram
    ...
    GS->>E: run chunk (LOCKSTYLE_CONFIG, UIConfig, REGION_CONFIG, lines 19-48)
    GS->>E: get_sets()
    E->>E: _G.LockstyleConfig/UIConfig/RECAST_CONFIG/BSTBeastPetData/BSTTPConfig (65-69)
    E->>M: include Mote-Include (71)
    M->>E: user_setup() (states, ecosystem, keybinds, UI, JCM gate, HUD, monitor start)
    M->>E: init_gear_sets() -> include sets file (254)
    E->>E: INIT_SYSTEMS (AutoMove starts), data_loader, message hooks (73-97)
    E->>E: JobChangeManager.cancel_all() (100-102)
    E->>F: include bst_functions.lua (106)
    F->>F: include 13 hook files, require dualbox_manager (100)
    E->>E: register_lockstyle_cancel("BST", cancel_bst_lockstyle_operations) (110-112)
```

`user_setup()` (`_master/entry/Tetsouo_BST.lua:125-227`):
...
2. `EcosystemManager.initialize()` builds `state.species` and `state.ammoSet`
   ...
   (`ecosystem_manager.lua` `EcosystemManager.initialize`). It must run before
   the HUD so `species` exists when the HUD first reads it.
3. `pcall(require, 'Tetsouo/config/bst/BST_KEYBINDS')` (which returns
   `KeybindManager.create('BST', ...)`, with `BST_CUSTOM.lua` and
   `COMMON_KEYBINDS.lua` keys appended), stored in the global `BSTKeybinds`,
   then `bind_all()` (8 BST binds plus the common ones, then `show_intro()`).
   A failed require prints the Lua error (since 2026-09-25).
4. `KeybindUI.smart_init("BST", UIConfig.init_delay)`, `_G.KeybindUI` exported.
5. JobChangeManager gate (172-196): since `KeybindManager` (2026-09-24),
   `bind_all()` -> `show_intro()` requires `BST_MACROBOOK` / `BST_LOCKSTYLE`,
   so `select_default_lockstyle` / `select_default_macro_book` already exist
   and the first branch runs: `JobChangeManager.initialize()` (no argument),
   the macro book, and the lockstyle after
   `LockstyleConfig.initial_load_delay`. The `else` branch (re-test in a 0.2 s
   coroutine) remains as a fallback.
...
(36-38), `BST_IDLE`, `BST_ENGAGED` (45-47), `BST_STATUS`, `BST_BUFFS` (54-56),
`BST_LOCKSTYLE`, `BST_MACROBOOK`, `BST_COMMANDS`, `BST_MOVEMENT` (64-68).
`BST_AFTERCAST` and `BST_PET_MIDCAST` require `pet_manager` and
`ready_move_categorizer` at include time (`BST_AFTERCAST.lua:18-33`,
`BST_PET_MIDCAST.lua:18`); the other logic modules load on first use.

AutoMove runs on BST. Nothing in the repo sets `_G.DISABLE_AUTOMOVE` today
(the readers are `INIT_SYSTEMS.lua:230` and `system_checker.lua`); commit 0563ff9 (2026-07-29)
removed it from the BST template because `state.Moving` had no other writer,
and the live entry does not set it either.

### Precast

`job_precast` (`BST_PRECAST.lua:133-168`):
...
- The cooldown check is skipped for every `spell.type == 'Monster'` action
  (142-150). Ready moves share recast id 102, the charge timer
...
- `WSPrecastHandler.handle` returns true for anything that is not a
  weaponskill (`ws_precast_handler.lua:48-49`), so it is called for every
  action.
...
- `prepare_ready_move` (119-125) stores the category on the spell table. The
...
- `job_post_precast` (175-180) only calls `WSPrecastHandler.apply_tp_gear`.
...
- Aftercast (`BST_AFTERCAST.lua:60-86`): only for `spell.type == 'Monster'`,
...
- Pet hooks: GearSwap fires `pet_midcast` when the pet readies a move
  ...
  (`BST_PET_MIDCAST.lua:30-44`) returns without `handled`, so Mote equips
...
- There is no pet precast event in GearSwap or Mote, so `job_pet_precast`
  (`BST_PET_PRECAST.lua:29-92`) is never called. Reward, Killer Instinct and
...
- Every Ready move also reaches the ability message handler. The moves are in
  ...
  the lookup misses the main/sub databases and stops there: type `Monster` is
  not in `DATABASE_TYPES`, so the other job databases are not walked
  (`ability_message_handler.lua` `find_ability_in_databases`, since
  `4a9b0b8`).

### Pet summon, jug selection, ecosystem

- `ecosystem` command -> `EcosystemManager.change_ecosystem()` (27-60): cycles
  ...
  refreshes the HUD.
- `species` command -> `change_species()` (66-103): cycles `state.species`,
  ...
  (`count_species_jugs`, 160-216).
- List order comes from `pairs()` over the pet table
  (`BST_PET_DATA.lua:124,145,166`), not from definition order; the comments
  there now say so. `species[1]` still matches `ammoSet[1]` because both lists
  are built by the same traversal and every species has exactly one pet in the
  data.
- `equip_pet_broth()` (108-132) calls `equip()` from a bare
...
- After Call Beast / Bestial Loyalty (and any `Monster` action that is not a
  ...
  `_G.start_pet_monitoring()` 2 s later (`BST_AFTERCAST.lua:92-98`). The
...
Mote first runs `job_midcast` (`BST_MIDCAST.lua:60-87`). Its early returns for
...
`job_post_midcast` (160-187):

- notifies `MidcastWatchdog` (it ignores non-magic, `midcast_watchdog.lua` `on_midcast_start`);
...
- dispatches Healing, Enhancing (with `MidcastManager.get_enhancing_target` and
  ...
  Magic to `MidcastManager.select_set` (97-145).

No `sets.midcast['<skill>']` exists in either sets file, and `select_set`
returns false when the skill's base set is missing
(`midcast_manager.lua:637-640`). Subjob spells therefore get no midcast gear, and
no fast cast either (`sets.precast.FC` is Mote's empty table). See
[midcast and buffs](../systems/midcast-and-buffs.md).

### Aftercast, idle, engaged, status, buffs

- `job_aftercast` (`BST_AFTERCAST.lua:47-103`): watchdog notify, Ready-move
  pet set (above), delayed monitor start. Everything else falls to Mote's
  `default_aftercast`. The old `gs c update` is gone (101-102).
- `customize_idle_set` -> `SetBuilder.build_idle_set` (`set_builder.lua:132-143`):
...
- `with_pdt` (55-61, through `pdt_overlay`, 43-48) combines `sets.<group>.<situation>.PDT`, else
...
- The pet-valid flag comes from `PetManager.update_pet_mode(_G.pet)`, cached
  ...
  between.
- `customize_melee_set` -> `build_engaged_set` (181-187, `engaged_for_situation`
  159-176): `engagedBoth` when
  the master is engaged and the pet is valid and engaged, else `sets.me.engaged`,
  with the PDT overlay, then weapons. The "pet only" branch (169-172) cannot
...
- `job_handle_equipping_gear` (`BST_MOVEMENT.lua:30-33`) is empty.

## Mote states

Created by `BSTStates.configure()` (`_master/config/bst/BST_STATES.lua:17-75`) on
every `user_setup()`; `species` and `ammoSet` are created by `EcosystemManager`.
Keybinds from `BST_KEYBINDS.lua:31-45`; `^` = Ctrl, `#` = Apps. `#numpad0`
(AutoMedicine) comes from the character's `config/COMMON_KEYBINDS.lua`. The
old "Alt+Numbers" comments are gone (`22e1816`).
...
| `PetEngaged` | 'false', 'true' (strings) | 'false' | none | `set_builder.lua:72,161`; written by the monitor, `engage_pet`/`disengage_pet` |
...
The `Moving` comment (`BST_STATES.lua:48-49`) now says AutoMove keeps it up to
date (fixed in `b6c7dc6`). The HUD
patterns in `UI_DISPLAY_BUILDER.lua:27,41,46` match the current names
(`Ecosystem`, `species`, `WeaponSet`, `SubSet`, `PetIdleMode`,
`AutoPetEngage`).

## Commands

`job_self_command` (`BST_COMMANDS.lua:143-482`) lowercases the first word and
...
|---------|--------|---------|
| `altjobupdate <job> <sub> ...` / `requestjob` | Dual-box job exchange (`altjobupdate` passes the sender name, 5th argument, since 2026-09-25) | 157-175 |
| `ui ...` | HUD commands | 181-185 |
| `debugmidcast` | Toggle `MidcastManager` debug | 190-200 |
| `debugprecast` | Toggle `_G.BST_DEBUG_PRECAST` (summon and pet-precast traces). Shadows the common `debugprecast` (`COMMON_COMMANDS.lua:622-623` -> `DEBUG_COMMANDS.lua` `handle_debugprecast`), which BST never reaches | 202-212 |
| `cyclestate <State>` | `CycleHandler.handle_cyclestate` (used by all state keybinds) | 221-226 |
| common commands | `reload`, `checksets`, `wa`, `wo`, `refill`, `am`, `alt*`, `lockstyle`, warp commands, ... | 243-252 -> `CommonCommands.handle_command(command, 'BST', table.unpack(args))` |
| `ecosystem` | `EcosystemManager.change_ecosystem()` | 258-266 |
| `species` | `EcosystemManager.change_species()` | 268-276 |
| `broth` / `broths` | Count items whose name contains "Broth" in inventory only (misses Crumbly Soil, Aged Humus, Gassy Sap, Windy Greens, T. Pristine Sap); no footer separator | 99-133, 278-282 |
| `pet engage` / `pet disengage` | `/pet "Fight" <t>` / `/pet "Heel" <me>` and set `PetEngaged` | 288-320 |
| `rdylist` | List the current pet's Ready moves, numbered by ability id | 326-361 |
| `rdymove N` | Pet engaged: `/pet "<move>" <me>`. Pet idle, player engaged: `Fight <stnpc>`, move after 3.5 s, `_G.bst_rdymove_active` for 4.5 s. Both idle: `Fight <stnpc>`, move after 3.5 s, `Heel` after 6 s, flag for 6.5 s | 363-481 |
...
`weapons.lua`, `pets.lua` merged into `sets` at `Tetsouo/sets/bst/bst_sets.lua:55-65`).

| Set | Looked up by | T | L |
|-----|--------------|---|---|
| `sets.idle`, `sets.engaged` | Mote base (fallback only) | 441, 501 | 152, 217 |
| `sets.me.idle`, `.PDT`, `.Town` (feet only used) | `set_builder.lua:83,91,121-123` | 442-444 | 153-155 |
| `sets.pet.idle`, `.PDT` | `set_builder.lua:78` | 453, 469 | 160, 176 |
| `sets.me.engaged`, `.PDT` | `set_builder.lua:165,174` | 486, 504 | 192, 206 |
| `sets.pet.engaged`, `.PDT` (built from `sets.pet.idle`) | `set_builder.lua:73,170` | 516, 533 | 220, 237 |
| `sets.pet.engagedBoth`, `.PDT` | `set_builder.lua:165` | 544, 561 | 248, 251 |
| `sets.me.PDT`, `sets.pet.PDT` | `with_pdt` fallback | absent (never needed) | absent |
| `sets['Aymur']`, `['Tauret']`, `["Agwu's Axe"]`, `['Adapa Shield']`, `['Diamond Aspis']`, `['Kraken Club']` | `set_builder.lua:98-104` | 211-217 | `Tetsouo/sets/bst/weapons.lua:22-30` |
| `sets['Blur Knife']` | nothing (not a `SubSet` value) | 214 | `Tetsouo/sets/bst/weapons.lua:27` |
| `sets['<pet name>']` x 25 (ammo only) | `BST_PRECAST.lua:100`, `equip_pet_broth` | 220-244 (all 25 match `BST_PET_DATA`) | `Tetsouo/sets/bst/pets.lua:19-43` |
...
| `sets.midcast.pet_{physical,physicalMulti,magicAtk,magicAcc}_moves` and `_ww` aliases | `BST_AFTERCAST.lua:70-78` | 647-697 | 334-387 |
...
| `sets.MoveSpeed` | `set_builder.lua:114-115` | 807 | 498 |
...
L 72-116). The categoriser overwrites them at load
(`ready_move_categorizer.lua:278-281`, loaded after the sets through
`BST_AFTERCAST.lua:30`) and nothing reads the globals. The 11 moves the
...
| `<char>/config/bst/BST_CUSTOM.lua` | nothing active | file | `KeybindManager` / `CustomStates` ([keybinds and custom states](../systems/keybinds-and-custom.md)) |
| `<char>/config/bst/BST_LOCKSTYLE.lua` `default`, `by_subjob`, `get_style` | 6 everywhere | file (20-59); factory fallback 1 (`shared/jobs/bst/functions/BST_LOCKSTYLE.lua:29`) | `LockstyleManager` ([factories](../systems/factories-and-helpers.md)) |
...
| `<char>/config/bst/BST_TP_CONFIG.lua` -> `_G.BSTTPConfig` | Moonshade 250, `fencer_jp_gifts = 4` | file | `WSPrecastHandler` -> `tp_bonus_calculator.lua` `effective_tp` |
...
`get_fencer_bonus` (`BST_TP_CONFIG.lua:98-163`) treats any sub item as
...
- The spell table is shared by precast and midcast but not by aftercast: carry
  data to aftercast by recomputing it, as `BST_AFTERCAST.lua:60-64` does.
...
- **Broth pre-equip never happens** (P3): `equip()` from a coroutine
  (`ecosystem_manager.lua:49-51,87-89,257-259`); the "Equipping" message is
  printed anyway.
...
- **Aftercast monitor start can error after a reload** (P3): the 2 s closure
  ...
  (`BST_AFTERCAST.lua:94-97`); `file_unload` sets it to nil.
- `BST_MIDCAST.lua:48` calls the undefined
...
- Fixed, no longer issues: the first Ready move after a load no longer loads
  every JA database (`4a9b0b8`); the Alt-bind comments of `BST_KEYBINDS`
  (`22e1816`); the AutoMove comment of `BST_STATES` and the list-order
  comments of `BST_PET_DATA` (`b6c7dc6`); `monitor_pet_after_command` removed
  (`b6c7dc6`); the `BST_PRECAST` "carried on the spell" comment now gives the
  real reason (2026-09-25).
- Dead code: `job_pet_precast` (whole `BST_PET_PRECAST.lua`),
  `PetManager.get_pet_status` / `is_pet_valid`,
  ...
  `sets.precast.JA['Misc Idle']` / `['Default']`, BST message wrappers with no
  caller.
